...
export function mediaColumnHasContent(column: unknown): boolean {
  ...
  const current = column as { media?: unknown; vimeoUrl?: unknown; content?: unknown }
  return Boolean(current.media) || Boolean(current.vimeoUrl) || hasLexicalText(current.content)
}
